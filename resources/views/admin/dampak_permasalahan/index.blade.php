                    ...
                    <div id="table-default" class="table-responsive">
                        <table id="example" class="table">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Nama Dampak Permasalahan</th>
                                    <th>Prioritas</th>
                                    {{-- <th>Akun</th> --}}
                            ...
                            <tbody class="table-tbody">
                                @foreach ($dampak_permasalahans as $dampak_permasalahan)
                                    <tr class="align-middle">
                                        <td>{{ $loop->iteration }}.</td>
                                        <td>{{ $dampak_permasalahan->nama_dampak }}</td>
                                        <td>{{ $dampak_permasalahan->prioritas }}</td>
                                        {{-- <td>
                                            @foreach ($dampak_permasalahan->akun as $user)
                                        ...
                                        <td class="text-center">
                                            <a href="#"
                                                onclick="handleDelete('{{ $dampak_permasalahan->id }}', '{{ $dampak_permasalahan->nama_dampak }}')"
                                            ...
                                            <a href="/dampak_permasalahan/{{ $dampak_permasalahan->id }}"
                                                class="btn btn-info btn-icon" title="Edit">
                                                <svg xmlns="http://www.w3.org/2000/svg"
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body text-center py-4 ">
                    ...
                    <div class="text-secondary">Apakah anda yakin ingin menghapus dampak permasalahan bernama <span
                            class="fw-bolder" id="nama"></span>
                        ?
                    </div>
                </div>
                <form action="/dampak_permasalahan" method="POST">
                    @csrf
                    @method('delete')
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Tambah Dampak Permasalahan</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="/dampak_permasalahan" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="row mb-3 align-items-end">
                            <div class="col">
                                <div class="mb-3">
                                    <label class="form-label required">Nama Dampak Permasalahan</label>
                                    <input type="text" class="form-control @error('nama_dampak') is-invalid @enderror"
                                        name="nama_dampak" placeholder="Masukan Nama Dampak Permasalahan"
                                        value="{{ old('nama_dampak') }}">
                                    @error('nama_dampak')
